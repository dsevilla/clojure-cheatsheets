...
%\DeclareCaptionFormat{listing}{\colorbox{yellow}{\parbox{\textwidth}{#1#2#3}}}
...
\begin{lstlisting}[label=ns,caption=Example Namespace Declaration,language=Lisp]
(ns my-cool-lib
  (:require [some-lib :as lib])
  (:use [another-lib :only (a-func)])
  (:require-macros [my.macros :as macs])
  (:use-macros [mo.macros :only (a-macro)]))
\end{lstlisting}
...
JS Direct Access: & \cmd{js/something} \\
